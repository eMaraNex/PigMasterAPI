...
class FeedingController {
  ...
  static async getByPig(req, res, next) {
    try {
      ...
      const records = await FeedingService.getFeedingByPig(pigId, {
        limit: parsedLimit,
        offset: parsedOffset,
        date_from,
        date_to,
  ...
  // Period records (weekly/monthly)
  static async createPeriodRecord(req, res, next) {
    try {
      const data = req.body;
      if (!data.farm_id && req.params?.farmId) {
        data.farm_id = req.params.farmId;
      ...
      const rec = await FeedingService.createPeriodRecord(data, userId);
      return SuccessResponse(
        res,
        201,
        "Period feeding record created successfully",
        rec,
      );
    } catch (error) {
      logger.error(`Create period feeding record error: ${error.message}`);
      next(error);
    }
  }

  static async getByFarm(req, res, next) {
    try {
      const { farmId } = req.params;
      const { limit, offset, date_from, date_to, record_type } = req.query;
      const parsedLimit = limit ? parseInt(limit, 10) : undefined;
      const parsedOffset = offset ? parseInt(offset, 10) : undefined;

      const records = await FeedingService.getFeedingByFarm(farmId, {
        limit: parsedLimit,
        offset: parsedOffset,
        date_from,
        date_to,
      ...
      logger.error(`Get feeding records by farm error: ${error.message}`);
      next(error);
    }
  }

  static async getPeriodRecordsByFarm(req, res, next) {
    try {
      const { farmId } = req.params;
      const { record_type, start_date, end_date } = req.query;
      const records = await FeedingService.getPeriodRecordsByFarm(farmId, {
        record_type,
        start_date,
        end_date,
      });
      return SuccessResponse(
        res,
        200,
        "Period feeding records retrieved successfully",
        records,
      );
    } catch (error) {
      logger.error(`Get period feeding records error: ${error.message}`);
      next(error);
    }
  }

  static async deletePeriodRecord(req, res, next) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      if (!userId) throw new ValidationError("User not authenticated");
      const rec = await FeedingService.deletePeriodRecord(id, userId);
      return SuccessResponse(
        res,
        200,
        "Period feeding record deleted successfully",
        rec,
      );
    } catch (error) {
      logger.error(`Delete period feeding record error: ${error.message}`);
      next(error);
    }
  }
  ...
  static async updateSchedule(req, res, next) {
    try {
      ...
      if (!userId) throw new ValidationError("User not authenticated");
      const data = req.body;
      const schedule = await FeedingService.updateFeedingSchedule(
        id,
        data,
        userId,
      );
