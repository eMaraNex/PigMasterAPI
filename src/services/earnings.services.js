...
class EarningsService {
    ...
    static async getEarningsById(id, farmId) {
        try {
            const result = await DatabaseHelper.executeQuery(
                `SELECT er.*, r.pig_id, r.name AS pig_name, h.id AS hutch_id
    ...
    static async updateEarnings(id, farmId, earningsData, userId) {
        ...
        try {
            if (type && !['pig_sale', 'urine_sale', 'manure_sale', 'other'].includes(type)) {
                throw new ValidationError('Type must be pig_sale, urine_sale, manure_sale, or other');
            }
