...
class FeedingService {
  ...
  static async _livePigCount(penId, farmId) {
    const r = await DatabaseHelper.executeQuery(
      `SELECT COUNT(*) AS cnt
         FROM pigs
        WHERE pen_id = $1 AND farm_id = $2 AND is_deleted = 0`,
      [penId, farmId],
    );
    return parseInt(r.rows[0]?.cnt ?? "0", 10);
  }

  static _normaliseRow(row) {
    if (!row) return row;
    ...
    if (row.pigs_count != null) row.pigs_count = parseInt(row.pigs_count, 10);
    return row;
  }
  ...
  static async createFeedingRecord(data, userId) {
    ...
    if (!pig_id && !pen_id) {
      throw new ValidationError(
        "Feeding records require either pig_id or pen_id",
      );
    }
    ...
    try {
      ...
      if (pen_id) {
        ...
        pigsCount = await FeedingService._livePigCount(pen_id, farm_id);
        if (pigsCount === 0)
          throw new ValidationError(
            "Cannot record feeding for a pen with no pigs",
          );
      }

      const result = await DatabaseHelper.executeQuery(
        `INSERT INTO feeding_records (
            id, pig_id, pen_id, farm_id,
            feed_type, amount, unit,
            feeding_time, fed_by, notes,
            record_type, pigs_count,
      ...
      return FeedingService._normaliseRow(result.rows[0]);
    } catch (error) {
  ...
  //  period feeding record (weekly / monthly) 
  ...
  static async createPeriodRecord(data, userId) {
    ...
    if (!pen_id) {
      throw new ValidationError("Period feeding records require a pen_id");
    }

    try {
      const penRow = await DatabaseHelper.executeQuery(
        "SELECT id FROM pens WHERE id = $1 AND farm_id = $2 AND is_deleted = 0",
        [pen_id, farm_id],
      );
      if (penRow.rows.length === 0)
        throw new ValidationError("Pen not found for provided farm");

      let pigsCount =
        clientPigsCount != null && parseInt(clientPigsCount, 10) > 0
          ? parseInt(clientPigsCount, 10)
          : await FeedingService._livePigCount(pen_id, farm_id);
      ...
      if (isNaN(totalAmountNumeric))
        throw new ValidationError("total_amount must be a valid number");
      ...
      const result = await DatabaseHelper.executeQuery(
        ...
        [
          ...
          record_type,
          feed_type,
          total_amount, // amount (varchar) for legacy compat
          totalAmountNumeric, // total_amount (numeric) for calculations
          unit,
          start_date,
          end_date,
          JSON.stringify(pigs_in_pen),
          pigsCount,
          start_date, // feeding_time placeholder for period rows
          userId || null,
          notes || null,
        ],
      );
      ...
      logger.info(
        `Period feeding record (${record_type}) created by user ${userId} ` +
          `for farm ${farm_id}, pen ${pen_id}, pigs_count ${pigsCount}`,
      );
      return FeedingService._normaliseRow(result.rows[0]);
    } catch (error) {
  ...
  static async getFeedingByFarm(
    ...
    try {
      ...
      return result.rows.map(FeedingService._normaliseRow);
    } catch (error) {
      logger.error(
    ...
    try {
      let query = `
        SELECT * FROM feeding_records
         WHERE farm_id = $1
           AND is_deleted = 0
           AND record_type IN ('weekly','monthly')`;
      const params = [farmId];
      let idx = 2;
    ...
    try {
      ...
      const result = await DatabaseHelper.executeQuery(
        `UPDATE feeding_records SET
            ...
            feeding_time = COALESCE($8,  feeding_time),
        ...
        [
          ...
          feed_type || null,
          amount || null,
          total_amount != null ? parseFloat(total_amount) : null,
          unit || null,
          feeding_time || null,
          start_date || null,
          end_date || null,
  ...
  /** Alias kept so existing period-record routes need no changes. */
  static async deletePeriodRecord(id, userId) {
    return FeedingService.deleteFeedingRecord(id, userId);
  }
    ...
    try {
      ...
      const result = await DatabaseHelper.executeQuery(
        `INSERT INTO feeding_schedules (
            id, pig_id, daily_amount, feed_type, times,
            special_diet, last_fed, is_active,
