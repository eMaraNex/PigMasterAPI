...
class RowsService {
    ...
    static async createRow(rowData, userId) {
        ...
        try {
            ...
            const hutchResults = await DatabaseHelper.executeQuery(
                `SELECT * FROM hutches WHERE row_id = $1`, [result.rows.id]
            )
            result.hutches = hutchResults.rows;
            logger.info(`Row ${result.rows.id} (${name}) created with ${hutches.length} hutches (${JSON.stringify(distribution)}) by user ${userId}`);
            return result;
        } catch (error) {
    ...
    static async deleteRow(rowId, farmId, userId) {
        try {
            // Check if row has hutches with pigs inside first. If they have pigs, cannot delete
            const pigs = await DatabaseHelper.executeQuery(
                `SELECT * FROM pigs
                INNER JOIN hutches h ON h.id = pigs.hutch_id AND h.row_id = $1
